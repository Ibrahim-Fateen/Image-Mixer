...
class Mixer:
    def __init__(self, images: [Image], region=None):
        ...
        """
        self.images = images
        self.mask = self.get_mask(region)
    ...
    def mix_mag_phase(self, weights: []):
        """
        :param weights: A list of weights dict for each image. {"Magnitude": 0.5, "Phase": 0.5}
        :return: Image
        """
        mag_weights = [weight["Magnitude"] for weight in weights]
        phase_weights = [weight["Phase"] for weight in weights]
        mixed_magnitude = self.mix_mag(mag_weights)
        mixed_phase = self.mix_phase(phase_weights)
    ...
    def mix_phase(self, weights):
        ...
        """
        phases = [image.get_phase() * self.mask for image in self.images]
    ...
    def mix_real(self, weights):
        pass

    def mix_imaginary(self, weights):
        pass
